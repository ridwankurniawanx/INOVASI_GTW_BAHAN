#!/usr/bin/env python3
# gateway_v10.0.py - Rilis Stabil
# Deskripsi: Gateway IEC 61850 ke IEC 60870-5-104.
# Fitur v10.0:
# - UI Web menampilkan data dikelompokkan per IED.
# - Status koneksi (CONNECTING, CONNECTED, DISCONNECTED) individual per IED.
# - Format IP:Port yang benar dan UI yang lebih rapi.
# - Menggunakan cache model IED untuk startup yang cepat setelah eksekusi pertama.
...
processing_queue, broadcast_queue, shutdown_event, iec104_server, main_loop = None, None, None, None, None
...
def start_http_server():
    ...
    web_dir = os.path.dirname(os.path.abspath(__file__)); os.chdir(web_dir)
    with socketserver.TCPServer(("", HTTP_PORT), NoCacheHandler) as httpd:
        logging.info(f"HTTP server menyajikan file dari '{web_dir}' di port {HTTP_PORT}"); httpd.serve_forever()

async def websocket_handler(websocket, path):
    logging.info(f"WebSocket client terhubung: {websocket.remote_address}")
    async with cache_lock:
        if connection_status_cache:
            status_tasks = [websocket.send(json.dumps(status)) for status in connection_status_cache.values()]
            await asyncio.gather(*status_tasks, return_exceptions=True)
        if realtime_data_cache:
            data_tasks = [websocket.send(json.dumps(data)) for data in realtime_data_cache.values()]
    ...
    try:
        ...
        logging.info(f"WebSocket client terputus: {websocket.remote_address}"); websocket_clients.remove(websocket)
...
async def broadcast_updates_task(queue):
    ...
    while not (shutdown_event and shutdown_event.is_set()):
        try:
            ...
            queue.task_done()
        except asyncio.CancelledError: logging.info("Broadcast task dihentikan."); break
        except Exception as e: logging.error(f"Error di dalam broadcast_updates_task: {e}", exc_info=True)
...
def process_data_update(ied_id, key, data):
    ...
    if "iec61850://" in reported_key:
        try:
            parsed_uri = urlparse(reported_key); mms_path_from_key = parsed_uri.path.lstrip('/')
...
async def data_processor_task(queue):
    ...
    while not (shutdown_event and shutdown_event.is_set()):
        try:
            ...
            if item.get('type') == 'process_data': await loop.run_in_executor(None, process_data_update, item['ied_id'], item['key'], item['data'])
            elif item.get('type') == 'invalidate': await loop.run_in_executor(None, do_invalidation, item['ied_id'])
            queue.task_done()
        except asyncio.CancelledError: break
        except Exception as e: logging.error(f"Error di dalam data_processor_task: {e}", exc_info=True)
...
async def ied_handler(ied_id, uris):
    ...
    def locked_check_state():
        with ied_locks[ied_id]:
            ...
            if conn_info.get('con'): return IedConnection_getState(conn_info['con']) == 2 # CON_STATE_CONNECTED = 2
            return False
    ...
    while not (shutdown_event and shutdown_event.is_set()):
        try:
            ...
            logging.error(f"[{ied_id}] Handler error: {e}.")
        finally:
...
async def main():
    ...
    logger = logging.getLogger('gateway-v10.0')
    ...
    logger.info("Gateway v10.0 (Rilis Stabil) dimulai")
...
def create_index_html():
    ...
    logging.info(f"Membuat/memperbarui file 'index.html' di '{file_path}'.")
...
<head>
    ...
    <style>
        ...
        h1 { color: #0056b3; }
        h2 { color: #343a40; border-bottom: 2px solid #007bff; padding-bottom: 5px; margin-top: 40px; display: flex; align-items: center;}
        table { border-collapse: collapse; width: 100%; margin-top: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); background-color: white;}
        ...
        .status { font-weight: bold; padding: 2px 8px; border-radius: 5px; color: white; display: inline-block; margin-left: 15px; min-width: 100px; text-align: center;}
...
<body>
    ...
    <script>
        ...
        function getIedId(data) {
            const source = data.address || data.ied_id;
            if (!source) return { domId: null, displayId: null };

            let displayId = source;
            const match = source.match(/iec61850:\/\/([^\/]+)/);
            if (match) {
                displayId = match[1];
            }
            
            const domId = displayId.replace(/[.:]/g, '-');
            return { domId, displayId };
        }

        function createOrUpdateIedContainer(domId, displayId, status) {
            const containerId = 'ied-container-' + domId;
            let container = document.getElementById(containerId);

            if (!container) {
                container = document.createElement('div');
                container.id = containerId;
                container.className = 'ied-group';
                container.innerHTML = `
                    <h2>
                        IED: ${displayId}
                        <span id="status-${domId}" class="status"></span>
                    </h2>
            ...
            const statusSpan = document.getElementById(`status-${domId}`);
        ...
        function connect() {
            ...
            socket.onopen = () => console.log("[open] Koneksi WebSocket berhasil dibuat.");
            socket.onmessage = function(event) {
                try {
                    const data = JSON.parse(event.data);
                    const { domId, displayId } = getIedId(data);
                    
                    if (!domId || domId === 'unknown-ied') return;

                    if (data.type === 'status_update') {
                        createOrUpdateIedContainer(domId, displayId, data.status);
                        return;
                    }
                    
                    if (data.type !== 'data_update' && data.type !== 'invalidation') return;

                    const tableBody = document.querySelector(`#table-${domId} tbody`);
                    if (!tableBody) return; // Jangan buat container dari pesan data

                    let row = tableBody.querySelector(`#ioa-${data.ioa}`);
                    if (!row) {
                        ...
                        let inserted = false;
                        for (const r of rows) {
                            if (newIOA < parseInt(r.id.split('-')[1], 10)) {
                                tableBody.insertBefore(row, r);
                                inserted = true;
                                break;
                            }
                        }
                    ...
                    const [_, addressCell, valueCell, timestampCell] = row.cells;
                ...
                } catch (error) { console.error("Gagal mem-parsing data JSON:", error); }
            };
