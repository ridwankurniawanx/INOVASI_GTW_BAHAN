#!/usr/bin/env python3
# gateway_v8.1.py - Specific Path Extraction & Adaptive Polling
# Deskripsi: Gateway IEC 61850 ke IEC 60870-5-104.
# Fitur v8.1: Mendukung path spesifik (misal: #mag.f) di config untuk presisi,
#             dengan fallback ke 'find_first_float' dan polling adaptif.
...
ied_locks = {}
ied_clients = {}
# PERUBAHAN: Tambahkan map untuk menyimpan path nilai spesifik
...
def find_first_float(data):
    """
    Mencari secara rekursif nilai float atau int pertama dalam struktur data.
    Ini adalah metode fallback jika tidak ada path spesifik yang diberikan.
    """
...
def get_value_by_path(data_dict, path_str):
    """
    FUNGSI BARU: Mengambil nilai dari nested dictionary berdasarkan path string (misal: 'mag.f').
    """
    keys = path_str.split('.')
    ...
    try:
        ...
        # Pastikan nilai akhirnya adalah numerik
...
def command_60870_callback(ioa, ioa_data, srv, select_value):
    ...
    uri_part = config_line.split('#')[0] # Ambil bagian URI saja
    try:
...
def process_data_update(ied_id, key, data):
    """
    PERUBAHAN BESAR: Fungsi ini sekarang memeriksa apakah ada path spesifik.
    Jika ada, gunakan get_value_by_path. Jika tidak, gunakan find_first_float.
    """
    if not isinstance(data, dict) or 'value' not in data: return
    reported_key, value_to_update = key, data['value']

    # Dapatkan path dasar dari URI yang dilaporkan
    ...
    # Temukan IOA yang cocok dengan path dasar
    ...
    for config_path, ioa in mms_to_ioa_map.items():
        if ioa in valid_ioas_for_ied and config_path.startswith(mms_path_from_key):
            # Cek apakah ada value path spesifik untuk config_path ini
            value_path = mms_to_value_path_map.get(config_path)
            final_value = None

            if value_path:
                # METODE 1: Gunakan path spesifik jika ada
                ...
                # METODE 2: Fallback ke find_first_float jika tidak ada path spesifik
            ...
            break # Hentikan pencarian setelah match pertama
...
# --- ASYNC TASKS (Tidak ada perubahan di ied_handler dan iec104_processor) ---
...
async def main():
    ...
    logger.info("Gateway v8.1 (Specific Path Extraction) started")
    ...
    for section in all_sections:
        if section in config:
            for ioa, config_line in config[section].items():
                # PERUBAHAN: Parsing untuk path spesifik dan inversi
                ...
                if '#' in uri_part:
                    uri_part, value_path = uri_part.split('#', 1)
                ...
                if section in data_types:
                    ...
                    if value_path:
                        mms_to_value_path_map[mms_path] = value_path
                ...
                if section in command_types:
                    # Simpan full config line untuk command
                    ioa_to_mms_config[ioa_int] = config_line
